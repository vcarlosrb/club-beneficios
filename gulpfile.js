//'use strict';
// generated on 2016-04-17 using generator-cg-gas 3.3.4
...
gulp.task('build:dist', ['clean'], function(cb) {
  runSequence([/*'jshint', */'build', 'copy', 'copy:assets','copy:partial','copy:directive','copy:modals','copy:modules', 'images'/*,'test:unit'*/], 'html', cb);
});
...
//copy partials in dist folder
gulp.task('copy:partial', function() {
    return gulp.src([
        config.base + '/partial/**/*.html',
...
//copy directives in dist folder
...
//copy modals in dist folder
gulp.task('copy:modals', function() {
    return gulp.src([
        config.base + '/modals/**/*.html',
    ]).pipe(gulp.dest(config.dist + '/modals'))
        .pipe($.size({
            title: 'copy:modals'
        }));
});

//copy modules in dist folder
